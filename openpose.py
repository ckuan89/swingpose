...
def pose_detect(frame,net,inheight=368):
    ...
    valid_pairs, invalid_pairs = getValidPairs(output,mapIdx=mapIdx,POSE_PAIRS=POSE_PAIRS,frameWidth=frameWidth, frameHeight=frameHeight,detected_keypoints=detected_keypoints)
    personwiseKeypoints = getPersonwiseKeypoints(valid_pairs, invalid_pairs,mapIdx=mapIdx,POSE_PAIRS=POSE_PAIRS,keypoints_list=keypoints_list)
    ...
    # plt.figure(figsize=[15,15])
    # plt.imshow(frameClone[:,:,[2,1,0]])
    return frameClone, personwiseKeypoints
...
def getValidPairs(output,mapIdx,POSE_PAIRS,frameWidth, frameHeight,detected_keypoints):
    valid_pairs = []
    invalid_pairs = []
    n_interp_samples = 10
    paf_score_th = 0.1
